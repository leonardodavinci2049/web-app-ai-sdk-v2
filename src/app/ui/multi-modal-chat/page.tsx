...
export default function MultiModalChatPage() {
  ...
  const getPartKeyBase = (part: (typeof messages)[number]["parts"][number]) => {
    switch (part.type) {
      case "text":
        return `text-${part.text}`;
      case "file":
        return `file-${part.url}-${part.mediaType ?? "unknown"}-${part.filename ?? "unnamed"}`;
      default:
        return part.type;
    }
  };
        ...
        return (
          <div key={message.id} className="mb-4">
            ...
            {message.parts.map((part) => {
              const partKeyBase = getPartKeyBase(part);
              const occurrence = partKeyCounts.get(partKeyBase) ?? 0;
              partKeyCounts.set(partKeyBase, occurrence + 1);
              const partKey = `${message.id}-${partKeyBase}-${occurrence}`;
